...
- **API limitation fix**: discovered the free/test API key caps `all_leagues.php` to ~10 Soccer-only entries (and `all_sports.php`/`search_all_leagues.php` results to a handful of rows each) - a tier restriction added by TheSportsDB after this assignment's brief was written. Worked around it by also querying `search_all_leagues.php?s=<sport>` for a fixed list of common sports (Basketball, Motorsport, Ice Hockey, Baseball, American Football, Rugby, Cricket, Tennis) in parallel and merging/deduplicating with the base `all_leagues.php` result, so the sport filter has real multi-sport data to demonstrate.
